...
export const login = async (req, res, next) => {
    ...
    const accessToken = generateToken({
        payload: { id: checkUser._id, email: checkUser.email }, secretKey: process.env.TOKEN_ACCESS_SECRET_KEY, option: {
            expiresIn: Number(process.env.TOKEN_ACCESS_EXPIRE_TIME),
            jwtid: uuid(),
        }
    })
    const refreshToken = generateToken({
        payload: { id: checkUser._id, email: checkUser.email }, secretKey: process.env.TOKEN_REFRESH_SECRET_KEY, option: {
...
export const refreshToken = async (req, res, next) => {
    ...
    const decode = verifyToken({ token: refreshtoken, secretKey: process.env.TOKEN_REFRESH_SECRET_KEY })

    const accessToken = generateToken({
        payload: { id: decode.id, email: decode.email }, secretKey: process.env.TOKEN_ACCESS_SECRET_KEY, option: {
            expiresIn: Number(process.env.TOKEN_ACCESS_EXPIRE_TIME),
            jwtid: uuid(),
        }
    })

    return successResponse({ res, message: "Token refreshed successfully", data: { accessToken } })
}
...
export const loginWithGmail = async (req, res, next) => {
    ...
    if (user) {
        if (user.provider === providerEnum.GOOGLE) {
            const accessToken = generateToken({
                payload: { id: user._id, email: user.email }, secretKey: process.env.TOKEN_ACCESS_SECRET_KEY, option: {
                    expiresIn: Number(process.env.TOKEN_ACCESS_EXPIRE_TIME),
                    jwtid: uuid(),
                }
            })
