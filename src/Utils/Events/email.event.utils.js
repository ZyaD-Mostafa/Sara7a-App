...
import { creativeMessageTemplateArabic } from "../Emails/newMessageEmailHTML.js";
...
emailEvent.on("newMessage", (data) => {
    sendEmail({
        to: data.to, subject: emailSubject.newMessage, html: creativeMessageTemplateArabic(data.inboxLink)
    }).then(() => console.log(`✅ Email sent to ${data.to}`))
        .catch(err => console.error("❌ Email sending failed:", err.message));
})